...
---
Nota sobre o Modo 1: A divergência observada na trajetória é inerente a sistemas de odometria diferencial, onde pequenos erros na leitura dos encoders ou patinagem física das rodas resultam em um erro angular acumulativo no Yaw (guinada). Como a posição $(x, y)$ é calculada através da integração de velocidade ao longo deste ângulo, qualquer desvio na orientação é propagado exponencialmente para o plano cartesiano. Diferente do Roll (balanço lateral) e do Pitch (inclinação frontal), que são menos críticos para a navegação plana, o erro no Yaw atua como um desvio na "bússola" interna do robô, fazendo com que ele erre a direção do movimento e, consequentemente, acumule erros de translação incontroláveis.
