...
---
Nota: O Ground Truth (/gt/odom) é utilizado estritamente para fins de avaliação e métricas comparativas. O Filtro de Kalman Estendido (EKF) não possui acesso a este tópico durante o processamento, garantindo que a estimativa de estado seja realizada exclusivamente com sensores embarcados. O Ground Truth opera no referencial global (World/Map), servindo como origem absoluta (0,0,0) para a validação da trajetória.Durante os testes do Modo 1, observou-se que a divergência na trajetória é inerente à odometria diferencial, onde erros de encoders resultam em um erro angular acumulativo no Yaw (guinada). Identificou-se que uma inversão no sinal da velocidade angular do frame base resultou em uma trajetória refletida (espelhada), confirmando que a cinemática do robô é altamente dependente da correta definição do sinal de Yaw em relação ao referencial global. Este comportamento ilustra como o erro angular, ao ser integrado no cálculo da posição (x, y), propaga-se exponencialmente no plano cartesiano, sendo o principal fator de deriva que o sistema de fusão sensorial (Modo 3) visa corrigir.
